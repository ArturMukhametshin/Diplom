...
def cakes(request):
    ...
    title = "Торты"
    return render(request, 'cakes.html', {'title': title})
...
def cupcakes(request):
    ...
    title = "Капкейки"
    return render(request, 'cupcakes.html', {'title': title})
...
def donuts(request):
    ...
    title = "Пончики"
    return render(request, 'donuts.html', {'title': title})


def national_sweets(request):
    """Данная функция принимает запрос, содержащий информацию о текущем HTML-запросе,
    и генерирует HTML-страницу Комерческие интерьеры. Предаёт переменную title в шаблон (доступна как title)"""
    title = "Национальные сладости"
    return render(request, 'national_sweets.html', {'title': title})
